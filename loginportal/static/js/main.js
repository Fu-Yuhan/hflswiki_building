...
$.fn.OnlyOn = function (type, selector, data, fun) {
    ...
    if (!is_oned) {
        ...
        console.log('äº‹ä»¶é‡å¤ç»‘å®š{typeï¼š' + type + ',selector:' + selector + ',data:' + data + '}');
    }
};
...
function countdown(_this) {
    ...
    function getTimeRemaining(endtime) {
        ...
        var endDate = new Date(endtime); //è®¾ç½®æˆªæ­¢æ—¶é—´
    ...
    function updateClock() {
        ...
        var html = t.days + 'å¤©' + t.hours + 'å°æ—¶' + t.minutes + 'åˆ†' + t.seconds + 'ç§’';
        _this.html(html);
        if (t.total <= 0) {
            clearInterval(timeinterval);
            var over_text = _this.attr('data-over-text') || 'å·²ç»“æŸ';
...
function new_dplayer() {
    var selector = '.new-dplayer:not(.dplayer-initialized),[data-dplayer]:not(.dplayer-initialized)'; //é€‰æ‹©å™¨
    if ($(selector).length) {
...
//è®©tabæ ç›®çš„é«˜äº®å…ƒç´ æ˜¾ç¤ºåœ¨ä¸­é—´
...
/**
 * @description: AJAXèŽ·å–åŒ…è£…å‡½æ•°
 * @param {*} _this ä¼ å…¥ç‚¹å‡»æŒ‰é’®çš„è‡ªå·±,éœ€è¦æœ‰hrefæˆ–ajax-hrefä¸‹ä¸€é¡µçš„é“¾æŽ¥(å¿…é¡»)
 * @param {*} con éœ€è¦æ’å…¥çš„çˆ¶å…ƒç´ é€‰æ‹©å™¨ (å¿…é¡»)
 * @param {*} jcon èŽ·å–å†…å®¹çš„çˆ¶å…ƒç´ é€‰æ‹©å™¨ (å¿…é¡»)
 * @param {*} item èŽ·å–çš„åˆ—è¡¨é€‰æ‹©å™¨   (å¿…é¡»)
 * @param {*} loader åŠ è½½åŠ¨ç”»çš„å†…å®¹ ï¼ˆéžå¿…é¡»ï¼Œæœ‰é»˜è®¤å€¼ï¼‰
 * @param {*} pag èŽ·å–çš„åˆ†é¡µå†…å®¹é€‰æ‹©å™¨ ï¼ˆå¿…é¡»ï¼‰
 * @param {*} next å¦‚æžœéœ€è¦é‡æ–°æ’å…¥ä¸‹ä¸€é¡µé“¾æŽ¥ï¼šèŽ·å–åˆ†é¡µå†…å®¹ä¸­çš„ä¸‹ä¸€é¡µ é€‰æ‹©å™¨
 * @param {*} trigger å°†èŽ·å–çš„ä¸‹ä¸€é¡µé“¾æŽ¥ä»Žæ–°æ’å…¥åˆ°çš„æ–°çš„ æŒ‰é’®ä¸­-çš„classå€¼
 * @param {*} replace æ›¿æ¢åˆ—è¡¨å†…å®¹è€Œä¸æ˜¯è¿½åŠ |'ajax-replace'
 * @param {*} nomore å…¨éƒ¨åŠ è½½å®Œæˆä¹‹åŽçš„æ–‡æ¡ˆ
 * @param {*} top å°†å†…å®¹è¿½åŠ åˆ°é¡¶éƒ¨è€Œä¸æ˜¯åº•éƒ¨
 * @return {*}
 */
function post_ajax(_this, con, jcon, item, loader, pag, next, trigger, replace, nomore, top, ajax_trigger) {
    ...
    jcon = jcon || con;
    ajax_trigger = ajax_trigger || (_win.ajax_trigger ? _win.ajax_trigger : '<i class="fa fa-arrow-right"></i>åŠ è½½æ›´å¤š');
    ...
    href &&
        $.ajax({
            ...
            success: function (a) {
                ...
                var c_c = $jcon.find(item); //åˆ—è¡¨
                var c_p = $jcon.find(pag); //ä¸‹ä¸€é¡µ
                var n_h = c_p.find(next).attr('href') || c_p.find(next).find('a').attr('href'); //ä¸‹ä¸€é¡µé“¾æŽ¥
                c_p = c_p.length ? c_p : '<div class="text-center mb20 padding-h10 muted-2-color no-more separator">' + nomore + '</div>'; //æ˜¯å¦è¿˜æœ‰ä¸‹ä¸€é¡µ
                ...
                if (trigger) {
                    //å°†èŽ·å–çš„ä¸‹ä¸€é¡µé“¾æŽ¥é‡æ–°æ’å…¥åˆ°çš„æ–°çš„ æŒ‰é’®ä¸­
                    c_p = 'undefined' != typeof n_h ? '<span class="post_ajax_trigger"><a class="' + trigger + '" href="' + n_h + '">' + ajax_trigger + '</a></span>' : c_p;
                }

                $con.find('.post_ajax_trigger,.post_ajax_loader').remove(); //åˆ é™¤è€çš„ä¸‹ä¸€é¡µé“¾æŽ¥/åŠ è½½åŠ¨ç”»
                if ($item.length) {
                    //åŽŸæœ¬æœ‰åˆ—è¡¨åˆ™è¿½åŠ
                    if (top) {
                        //è¿½åŠ æ–¹å‘ä¸ºé¡¶éƒ¨
...
_win.bd.on('click', '.ajax-next', function (e) {
    ...
    var _loader = '<div class="text-center muted-2-color mt20"><i class="loading mr10"></i>åŠ è½½ä¸­...</div>';
...
if (_wid < 768 && $('.drawer-sm').length) {
    ...
    _win.url_request = window.parent.location.search; //å…¼å®¹æ”¯ä»˜çš„GetRequestå‡½æ•°
    setTimeout(function () {
...
_win.bd.on('click', '.oauth-untying', function () {
    if (confirm('ç¡®è®¤è¦è§£é™¤è´¦å·ç»‘å®šå—ï¼Ÿ') == true) {
        var _this = $(this),
            data = {
...
_win.bd.on('click', '.medal-add-link', function () {
    if (confirm('ç¡®è®¤è¦æŽˆäºˆæ­¤å¾½ç« å—ï¼Ÿ') == true) {
...
_win.bd.on('click', '.medal-remove-link', function () {
    if (window.confirm('ç¡®è®¤è¦æ”¶å›žæ­¤å¾½ç« å—ï¼Ÿ')) {
        zib_ajax($(this));
        return false;
    }
});
...
function z_auto_fun() {
    ...
    //æµ·æŠ¥åˆ†äº«
    $('[poster-share]').length && tbquire(['poster-share']); //æµ·æŠ¥åˆ†äº«

    //äººæœºéªŒè¯
    $('[machine-verification]').length && tbquire(['captcha']); //äººæœºéªŒè¯
...
$.fn.autoAffix = function (top, bottom, auto_m) {
    ...
    if (!_this.length) return;
    var _body = $('body');
    var offset_top = ~~_this.attr('offset-top'); //åç§»
...
_win.bd.on('click', '[data-action]', function () {
    ...
    if (!_win.is_signin) {
        var t = lcs.get(_type) || '';
        if (-1 !== t.indexOf(',' + s + ',')) return notyf('å·²èµžè¿‡æ­¤' + (type == 'like' ? 'æ–‡ç« ' : 'è¯„è®º') + 'äº†ï¼', 'warning');
    ...
    action_ajax(_this, data, s, type, 'å·²èµžï¼æ„Ÿè°¢æ‚¨çš„æ”¯æŒ');
});
...
_win.bd.on('click', '.qrcode-signin', function () {
    ...
    $.post(
        ...
        null,
        function (n) {
            n || notyf('äºŒç»´ç èŽ·å–å¤±è´¥ï¼Œè¯·ç¨åŽå†è¯•', 'danger');
 ...
 * @param {*} callback å‡½æ•°
 * @param {*} delay æ—¶é—´
 * @param {*} immediate  æ˜¯å¦ç«‹å³æ‰§è¡Œ ä¸ºtrueåˆ™è®¡æ—¶å¼€å§‹å°±å°±æ‰§è¡Œ
 * @return {*}
 */
// å®šä¹‰ä¸€ä¸ªdebounceå‡½æ•°ï¼Œç”¨äºŽå‡½æ•°é˜²æŠ–
function debounce(callback, delay, immediate) {
    // å®šä¹‰ä¸€ä¸ªå˜é‡ï¼Œç”¨äºŽå­˜æ”¾å®šæ—¶å™¨
    var timeout;
    // è¿”å›žä¸€ä¸ªå‡½æ•°
    return function () {
        // å®šä¹‰ä¸€ä¸ªå˜é‡ï¼Œç”¨äºŽå­˜æ”¾this
        var context = this,
            // å®šä¹‰ä¸€ä¸ªå˜é‡ï¼Œç”¨äºŽå­˜æ”¾å‚æ•°
            args = arguments;
        // å®šä¹‰ä¸€ä¸ªå‡½æ•°ï¼Œç”¨äºŽæ¸…é™¤å®šæ—¶å™¨
        var later = function () {
            // å°†å®šæ—¶å™¨ç½®ä¸ºnull
            timeout = null;
            // å¦‚æžœä¸æ˜¯ç«‹å³æ‰§è¡Œï¼Œåˆ™æ‰§è¡Œå›žè°ƒå‡½æ•°
            if (!immediate) {
                callback.apply(context, args);
            }
        };
        // å¦‚æžœæ˜¯ç«‹å³æ‰§è¡Œï¼Œä¸”å®šæ—¶å™¨ä¸å­˜åœ¨ï¼Œåˆ™ç«‹å³æ‰§è¡Œå›žè°ƒå‡½æ•°
        var callNow = immediate && !timeout;
        // æ¸…é™¤å®šæ—¶å™¨
        clearTimeout(timeout);
        // é‡æ–°è®¾ç½®å®šæ—¶å™¨
        timeout = setTimeout(later, delay);
        // å¦‚æžœæ˜¯ç«‹å³æ‰§è¡Œï¼Œåˆ™ç«‹å³æ‰§è¡Œå›žè°ƒå‡½æ•°
        if (callNow) {
            callback.apply(context, args);
        }
    };
    ...
    if (multiple) {
        ...
        if (!_this.hasClass('active')) {
            ...
            if (_this.siblings('.active').length >= multiple) {
                return notyf('æœ€å¤šå¯é€‰æ‹©' + multiple + 'ä¸ª', 'danger');
            }
        }
...
_win.bd.on(
    ...
    debounce(function () {
        ...
        if (_this.val().length < 1) {
            ...
            text = _this.attr('warning-max') || 'æœ€å¤§å¯è¾“å…¥1$';
            _warning.html(text.replace('1$', max)).fadeIn(150);
        } else if (val < min) {
            text = _this.attr('warning-min') || 'æœ€å°å¯è¾“å…¥1$';
...
$.fn.AutoSearch = function () {
    return this.each(function () {
        ...
        var min_length_remind = 'è¯·è‡³å°‘è¾“å…¥' + min_length + 'ä¸ªå­—ç¬¦';
        var loading_icon = '<i class="loading em12"></i>';
        var loading_remind = loading_icon + '<span class="ml6">æ­£åœ¨æœç´¢ï¼Œè¯·ç¨å€™...</span>';
        var is_on = 'is-on';
...
_win.bd.on('click', '.trash-history-search', function () {
    if (confirm('ç¡®è®¤è¦æ¸…ç©ºå…¨éƒ¨æœç´¢è®°å½•ï¼Ÿ') == true) {
        $('.history-search')
            .slideUp()
            .delay(1000, function () {
                $(this).remove();
...
function zib_ajax(_this, data, success, noty, no_loading) {
    ...
    if (window.captcha) {
        ...
        window.captcha = {}; //åªèƒ½ä½¿ç”¨ä¸€æ¬¡
    }

    var _text = _this.html();
    var _loading = no_loading ? _text : '<i class="loading mr6"></i><text>è¯·ç¨å€™</text>';
    noty != 'stop' && notyf(noty || 'æ­£åœ¨å¤„ç†è¯·ç¨åŽ...', 'load', '', 'wp_ajax');
    ...
    $.ajax({
        ...
        dataType: 'json',
        error: function (n) {
            var _msg = 'æ“ä½œå¤±è´¥ ' + n.status + ' ' + n.statusText + 'ï¼Œè¯·åˆ·æ–°é¡µé¢åŽé‡è¯•';
            if (n.responseText && n.responseText.indexOf('è‡´å‘½é”™è¯¯') > -1) {
                _msg = 'ç½‘ç«™é‡åˆ°è‡´å‘½é”™è¯¯ï¼Œè¯·æ£€æŸ¥æ’ä»¶å†²çªæˆ–é€šè¿‡é”™è¯¯æ—¥å¿—æŽ’é™¤é”™è¯¯';
            }
            console.error('ajaxè¯·æ±‚é”™è¯¯ï¼Œé”™è¯¯ä¿¡æ¯å¦‚ä¸‹ï¼š', n);
        ...
        success: function (n) {
            ...
            if (noty != 'stop') {
                notyf(n.msg || 'å¤„ç†å®Œæˆ', ys, '', 'wp_ajax');
            } else if (n.msg) {
                notyf(n.msg, ys);
            }

            _this.attr('disabled', false).html(_text).trigger('zib_ajax.success', n); //å®Œæˆ
...
//AJAXæ‰§è¡Œå®ŒæˆåŽè‡ªåŠ¨åˆ‡æ¢åˆ°ä¸‹ä¸€ä¸ªtab
...
function posts_limit_height() {
    if ($('.limit-height').length) {
        ...
        if (~~r_h >= ~~r_m + 79) {
            var nn = '<div class="read-more"><a href="javascript:;" class="read-more-open">å±•å¼€é˜…è¯»å…¨æ–‡<i class="fa ml10 fa-angle-down"></i></a></div>';
...
//é¡µé¢åŠ è½½ä¹‹åŽè‡ªåŠ¨ç‚¹å‡»
$('[load-click]').attr('no-scroll', true).click();

//å›¾ç‰‡å»¶è¿Ÿæ‡’åŠ è½½-iasè‡ªåŠ¨åŠ è½½
...
//æ€§èƒ½æ£€æµ‹ï¼Œé€šè¿‡æµè§ˆå™¨å¸§çŽ‡
function fps_yh() {
    var lastTime = 0;
    var frames = 0;
    var fps = 0;
...
//é¡µé¢åŠ è½½å®Œæ¯•ä¹‹åŽåŠ è½½çš„å‡½æ•°
$(document).ready(function () {
    setTimeout(function () {
        //é¡µé¢åŠ è½½å®Œæ¯•ä¹‹åŽå†å»¶è¿Ÿä¸¤ç§’åŠ è½½çš„å‡½æ•°

        //æ£€æµ‹placeholder_scaleæ“ä½œ
        ...
        setTimeout(function () {
            //èŽ·å–ç™»å½•ç”¨æˆ·æ•°æ®ï¼Œå»¶è¿Ÿ2ç§’æ‰§è¡Œ
            $.get(_win.ajax_url, {
